    ...
    executions['BUY'] = dict(
        filter(
            lambda x: datetime.now().timestamp() - x[0] < bitflyer_params[
                'deltaTime'], executions['BUY'].items()))
    executions['SELL'] = dict(
        filter(
            lambda x: datetime.now().timestamp() - x[0] < bitflyer_params[
                'deltaTime'], executions['SELL'].items()))
    ...
    while True:
        try:
            ...
            print('%dsec Volume - Buy %f  Sell %f' %
                  (bitflyer_params['deltaTime'], buy_volume, sell_volume))

            # (2) Caluculate Total volume
            difference = math.fabs(
                math.sqrt(buy_volume) - math.sqrt(sell_volume))
            print('%dsec Volume - Difference %f' %
                  (bitflyer_params['deltaTime'], difference))
            ...
            if bitflyer_params[
                    'orderfilter'] < difference and current_status in ok_status:
                order_size = bitflyer_params['ordersize']
                if buy_volume > sell_volume:
                    order_price = bids_and_asks['bids'][0][
                        'price'] - bitflyer_params['profitspread']
                    order_side = 'BUY'
                else:
